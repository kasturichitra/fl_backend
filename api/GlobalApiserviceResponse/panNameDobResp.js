...
const PANNameDobActiveServiceResponse = async (
  data,
  services = [],
  index = 0,
) => {
  console.log("PANNameDobActiveServiceResponse called");
  ...
  if (!newService) {
    ...
    return PANNameDobActiveServiceResponse(data, services, index + 1);
  }

  const serviceName = newService.providerId || "";
  console.log(
    `[PANNameDobActiveServiceResponse] Trying service with priority ${index + 1}:`,
    newService,
  );

  try {
    const res = await panNameDobApiCall(data, serviceName, 0);
    ...
    console.log(
      `[PANNameDobActiveServiceResponse] ${serviceName} responded failure. Data: ${JSON.stringify(res)} → trying next service`,
    );
    return PANNameDobActiveServiceResponse(data, services, index + 1);
  } catch (err) {
    console.log(
      `[PANNameDobActiveServiceResponse] Error from ${serviceName}:`,
      err.message,
    );
    return PANNameDobActiveServiceResponse(data, services, index + 1);
  }
...
const panNameDobApiCall = async (data, service) => {
...
module.exports = {
  PANNameDobActiveServiceResponse,
};
