const { businessServiceLogger, otherServiceLogger } = require("../../Logger/logger");
const {
...
const NameMatchActiveServiceResponse = async (
  data,
  services = [],
  index = 0,
  tnId = "",
  ...
  console.log(
    "[NameMatchActiveServiceResponse] incoming data ===>>",
    JSON.stringify(data),
  );
  ...
  console.log(
    `[NameMatchActiveServiceResponse] Trying service with priority ${index + 1}:`,
    newService,
  );
  businessServiceLogger.info(
    `[NameMatchActiveServiceResponse] Trying service with priority ${index + 1}:`,
  ...
  try {
    const res = await NameMatchApiCall(data, serviceName, tnId);

    otherServiceLogger.info(
      `[NameMatchActiveServiceResponse] ${serviceName} responded failure. Data: ${JSON.stringify(res)} → trying next service`,
    );
    ...
    otherServiceLogger.info(
      `[NameMatchActiveServiceResponse] Error from ${serviceName}:`,
      err.message,
    );
    return NameMatchActiveServiceResponse(data, services, index + 1);
  }
...
const NameMatchApiCall = async (data, service, tnId) => {
  ...
  try {
    if (service === "TRUTHSCREEN") {
      ...
      console.log(
        "[NameMatchApiCall] TruthScreen API response:",
    ...
    console.log(
      `[NameMatchApiCall] ${service} API response:`,
