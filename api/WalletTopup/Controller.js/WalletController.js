  ...
  try {
    ...
    if (existingToken && existingToken.expiresAt) {
      const expiresAtSeconds = Math.floor(
        existingToken.expiresAt.getTime() / 1000,
      );
    ...
    const base64Auth = Buffer.from(`${CLIENT_CODE}:${SECRET_KEY}`).toString(
      "base64",
    );
...
exports.webhookForupi = async (req, res) => {
  ...
  try {
    ...
    /*
    ==========================================
    4. EMIT TO ORDER ROOM
    ==========================================
    */

    io.to(orderId).emit("payment_update", {
      orderId,
      paymentStatus,
      message: "Live status update",
      time: new Date(),
    });
    ...
    sockets.forEach((s) => s.leave(orderId));
