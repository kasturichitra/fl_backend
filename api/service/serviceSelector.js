...
async function selectService(servicecategory, serviceName,client='',req) {
    ...
    try {
        const FinalService = await axios.get(`${process.env.SUPERADMIN_URL}/api/v1/apimodule/getAllProvidersByService?serviceId=${serviceName}&categoryId=${servicecategory}`,{ headers: headers })
