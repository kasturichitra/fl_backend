...
const passportOcrVerifyServiceResponse = async (
  data,
  services = [],
  index = 0,
  client,
) => {
  console.log("passportOcrVerifyServiceResponse called");
  if (index >= services?.length) {
    return { success: false, message: "All services failed" };
  }
  ...
  if (!newService) {
    console.log(`No service with priority ${index + 1}, trying next`);
    return passportOcrVerifyServiceResponse(data, services, index + 1);
  }

  const serviceName = newService.providerId || "";
  console.log(
    `[passportOcrVerifyServiceResponse] Trying service with priority ${index + 1}:`,
    newService,
  );

  try {
    const res = await passportOcrApiCall(data, serviceName, client);
    ...
    console.log(
      `[passportOcrVerifyServiceResponse] ${serviceName} responded failure. Data: ${JSON.stringify(res)} → trying next service`,
    );
    return passportOcrVerifyServiceResponse(data, services, index + 1);
  } catch (err) {
    console.log(
      `[passportOcrVerifyServiceResponse] Error from ${serviceName}:`,
      err.message,
    );
    return passportOcrVerifyServiceResponse(data, services, index + 1);
  }
};
const passportOcrApiCall = async (data, service, CID) => {
  const tskId = generateTransactionId(12);
