...
const accountPennyDropSerciveResponse = async (
  data,
  services = [],
  index = 0,
  client = "",
  ...
  console.log(
    `[accountPennyDropSerciveResponse] Trying service with priority ${index + 1}:`,
    newService,
  );
  ...
  try {
    ...
    console.log(
      `[accountPennyDropSerciveResponse] ${serviceName} responded failure. Data: ${JSON.stringify(res)} → trying next service`,
    );
    return accountPennyDropSerciveResponse(data, services, index + 1);
  } catch (err) {
    console.log(
      `[accountPennyDropSerciveResponse] Error from ${serviceName}:`,
      err.message,
    );
    return accountPennyDropSerciveResponse(data, services, index + 1);
  }
...
const accountPennyDropApiCall = async (data, service, CID = "") => {
  ...
  try {
    ...
    console.log(
      `[accountPennyDropApiCall] API Error in ${service}:`,
      error.message,
    );
  ...
  console.log(
    `[accountPennyDropApiCall] ${service} Response Object:`,
    JSON.stringify(obj),
  );
  ...
  switch (service) {
    ...
    case "TRUTHSCREEN":
      if (
        obj?.status == 0 &&
        typeof obj?.msg?.description === "string" &&
        (obj?.msg?.description?.toLowerCase().includes("failed") ||
          obj?.msg?.status?.toLowerCase().includes("invalid"))
      ) {
        bankServiceLogger.info(`no record for this client: ${CID}`);
        return {
          success: false,
          data: {
            result: "NoDataFound",
      ...
      if (
        obj?.status == 0 &&
        typeof obj?.msg?.description === "string" &&
        obj?.msg?.message?.toLowerCase().includes("some technical")
      ) {
        return {
          success: false,
          data: null,
    ...
    case "CASHFREE":
      if (
        ...
        obj?.account_status?.toLowerCase().includes("invalid")
      ) {
        return {
          success: false,
          data: null,
